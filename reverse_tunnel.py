...
DEFAULT_SECONDS_BETWEEN_SSH_TESTS = 600

class SSHReverseTunnelManager:
    def __init__(self, remote_host, remote_saw_script, remote_test_script, local_port, remote_port, seconds_between_ssh_tests):
        self.remote_host = remote_host
        self.remote_saw_script = remote_saw_script
        self.remote_test_script = remote_test_script
        self.local_port = local_port
        self.remote_port = remote_port
        self.seconds_between_ssh_tests = seconds_between_ssh_tests
    ...
    def _run_reverse_tunnel(self):
        """Open and maintain the reverse SSH tunnel.
    ...
    def _monitor_connection(self):
        """Monitor the reverse tunnel connection and restart it if the test fails.
        ...
        while not self._stop_event.is_set():
            ...
            try:
                ...
                if result.returncode != 0:
                    print(f"Connection test failed at {time.ctime()}. Output: {result.stdout}{result.stderr}")
                    self._kill_reverse_tunnel_process()
                else:
                    print(f"Connection test successful at {time.ctime()}. Output: {result.stdout.strip()}")
            ...
            # Wait, checking for stop event periodically to allow shutdown
            for _ in range(self.seconds_between_ssh_tests):
                if self._stop_event.is_set():
                    break
                time.sleep(1)

    def _kill_reverse_tunnel_process(self):
        """Kill the reverse tunnel SSH process (if it's running)."""
    ...
    def start(self):
        """Start the two threads for managing the SSH reverse tunnel."""
    ...
    def stop(self):
        """Signal the threads to stop and attempt to terminate the SSH process."""
        if not self._stop_event.is_set():
...
def get_parsed_args():
    ...
    parser.add_argument(
        "--seconds-between-ssh-tests",
        type=int,
        default=DEFAULT_SECONDS_BETWEEN_SSH_TESTS,
        help="How long to wait between testing ssh connections."
    )
...
def main():
    """Parse arguments and start the SSHReverseTunnelManager."""
    ...
    manager = SSHReverseTunnelManager(
        ...
        seconds_between_ssh_tests=args.seconds_between_ssh_tests,
    )
